...
public class HTTPService {
	...
	private class MineServlet extends HttpServlet {
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			//挖矿自己产生的交易也需要同步给其它p2p节点
			List<Transaction> newTransactions = newBlock.getTransactions();
			for (Transaction tx: newTransactions){
				if (tx.coinbaseTx()){
					Transaction[] txs = {tx};
					String transactionMsg = JSON.toJSONString(new Message(P2PService.RESPONSE_TRANSACTION, JSON
							.toJSONString(txs)));
					p2pService.broatcast(transactionMsg);
					break;
				}
			}
		}
